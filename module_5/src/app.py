"""
Flask web application for graduate school application data analysis.

This module provides a web interface for viewing and analyzing graduate school
application data. It includes routes for displaying analytics, updating data,
and running the full data pipeline for scraping and processing new entries.
"""
import subprocess
    ...
    try:
        ...
        with psycopg.connect(conn_str) as conn:
            ...
            if new_entries_count > 0:
                ...
                command_args = ["python", "src/llm_hosting/app.py",
                                "--file", "new_structured_entries.json"]
        ...
        print(f"Database error occurred in the pipeline: {e}")
    except FileNotFoundError as e:
        print(f"File not found error in the pipeline: {e}")
    finally:
...
def analysis():
    ...
    try:
        ...
        print(f"Database error during page load query: {e}")
...
def update_analysis():
    ...
    try:
        ...
        print(f"Database error during analysis update query: {e}")
        return jsonify({"error": "Error loading data from the database."}), 500
    except ValueError as e:
        print(f"Value error during analysis update: {e}")
