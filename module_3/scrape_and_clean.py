...
def main(conn):
    ...
    if not rp.can_fetch(USER_AGENT, TARGET_URL):
        ...
        return 0 # Return 0 as no entries were scraped

    latest_date = get_most_recent_date(conn)
    if not latest_date:
        latest_date = date(2020, 1, 1)
        print(f"No existing data found. Starting initial scrape from {latest_date}.")

    new_data = scrape_and_clean(latest_db_date=latest_date)

    # Save new data to the output file and return the count
    if new_data:
        ...
        return len(new_data) # Return the number of entries found
    else:
        print("No new entries were found.")
        return 0 # Return 0 if no new data

def get_most_recent_date(conn):
    """ This was added to get the latest date from the database"""
    try:
        with conn.cursor() as cur:
            cur.execute("SELECT MAX(date_added) FROM applicants;")
            result = cur.fetchone()
            if result and result[0]:
                print(f"Most recent entry in database: {result[0]}")
                return result[0]
            return None
    except psycopg.Error as e:
        print(f"Database error while fetching most recent date: {e}")
        return None

def scrape_and_clean(latest_db_date=None, page_limit=5000):
    ...
    for page_num in range(1, page_limit + 1):
        ...
        for i, row in enumerate(main_rows):
            date_added = corrected_dates[i]
            if not date_added: continue
            
            if latest_db_date and datetime.strptime(date_added, '%Y-%m-%d').date() <= latest_db_date:
                stop_scraping = True
                print(f"Found entry from {date_added}, which is on or before the cutoff date ({latest_db_date}). Stopping scrape.")
                break
            ...
            ref_year = int(date_added.split('-')[0]) if date_added else None
...
if __name__ == '__main__':
    ...
    # Establish its own connection only when run directly
    with psycopg.connect(DB_CONN_STR) as connection:
        # Call the main pipeline function with the new connection
        main(connection)